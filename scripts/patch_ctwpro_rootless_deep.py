#!/usr/bin/env python3
"""Replace CTW Pro's extend.bin load with the local deep-patch dylib."""
...
NEW_PATH = b"@executable_path/fix.dylib"
...
NEW_COMMAND = dylib_command(LC_LOAD_DYLIB, NEW_PATH)
...
def verify_patched(data: bytes) -> None:
    ...
    if OLD_PATH in data:
        raise ValueError("the extend.bin load path is still present")
    if data.count(NEW_PATH) != 1:
        raise ValueError(f"unexpected fix.dylib load path count: {data.count(NEW_PATH)}")

    matches = [item for item in load_dylibs(data) if item[2] in {OLD_PATH, NEW_PATH}]
    expected = [(LOAD_COMMAND_OFFSET, LC_LOAD_DYLIB, NEW_PATH)]
    if matches != expected:
        raise ValueError(f"unexpected deep-patch load-command state: {matches!r}")
