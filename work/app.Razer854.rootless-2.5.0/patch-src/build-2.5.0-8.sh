...
PKGROOT="$WORK/pkgroot-2.5.0-8"
REPACK="$WORK/repack-2.5.0-8"
VERIFY="$WORK/verify-final-2.5.0-8"
OUT="$ROOT/patched/2.5.0_Razer雷蛇(无根)_2.5.0-8_app.Razer854.rootless_authstate_ustar.deb"
HOOK_SRC="$WORK/patch-src/RazerAuth2099Hook.m"
HOOK="$WORK/patch-src/RazerAuth2099.dylib.v8"
...
sed -i '' 's/^Version: .*/Version: 2.5.0-8/' "$PKGROOT/DEBIAN/control"

# Restore the original VM trampoline prologues. The authorization predicate is
# supplied by RazerAuth2099; UI methods and business callback order stay intact.
